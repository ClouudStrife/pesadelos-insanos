...
void Player::atirar(Lista<Projetil>& listaProjetil, Ponto ponto_mouse, bool click){
	if(!tempoRecarga.estaAtivo()){
		switch(arma){
			case pistola:
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).normalizado()*8, 5, 5, circular, 5, al_map_rgb(0, 0, 0)));
				break;
			case shotgun:
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).normalizado()*12, 5, 5, circular, 10, al_map_rgb(0, 0, 0)));
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).rotacionado(0.3).normalizado()*12, 5, 10, circular, 5, al_map_rgb(0, 0, 0)));
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).rotacionado(-0.3).normalizado()*12, 5, 10, circular, 5, al_map_rgb(0, 0, 0)));
				break;
			case sub:
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).normalizado()*12, 5, 5, circular, 10, al_map_rgb(0, 0, 0)));
				break;
			case rpg:
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).normalizado()*12, 20, 20, circular, 50, al_map_rgb(0, 0, 0)));
				break;
			case sniper:
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).normalizado()*15, 6, 6, circular, 65, al_map_rgb(0, 0, 0)));
				break;
			case laser:
				listaProjetil.insereNoInicio(Projetil(pos, Vetor(pos, ponto_mouse).normalizado()*15, 50, 7, retangular, 100, al_map_rgb(0, 0, 155)));
				break;
		}
		tempoRecarga.start();
	}
...
void Player::setArma(int arma){
    ...
    switch(arma){
    	case pistola:
    		...
    		break;
    	case shotgun:
    		tempoRecarga.setMaximo(1.2*60);
    		break;
    	case sub:
    		tempoRecarga.setMaximo(0.1*60);
    		break;
    	case rpg:
    		tempoRecarga.setMaximo(2*60);
    		break;
    	case sniper:
    		tempoRecarga.setMaximo(2*60);
    		break;
    	case laser:
    		tempoRecarga.setMaximo(1*60);
    		break;
	}
	tempoRecarga.start();
}
