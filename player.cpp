...
void Player::desenhar(bool movimento, bool mostrarHitbox){
	...
	switch(arma){
        ...
        case sniper:
			if(!movimento){
				al_draw_tinted_scaled_rotated_bitmap_region(spriteSniper, 0, 0, 63, 69,
					al_map_rgb(255, 255, 255), 18, 34.5, pos.getX(), pos.getY(), 0.5, 0.5, -angulo, 0);
			}
			else{
				al_draw_tinted_scaled_rotated_bitmap_region(spriteSniper, timerAnimacaoAndar.getContador()/3*63, 0, 63, 69,
					al_map_rgb(255, 255, 255), 18, 34.5, pos.getX(), pos.getY(), 0.5, 0.5, -angulo, 0);
				if(!timerAnimacaoAndar.estaAtivo())
			       timerAnimacaoAndar.start();
			}
        ...
        case laser:
			if(!movimento){
				al_draw_tinted_scaled_rotated_bitmap_region(spriteLaser, 0, 0, 63, 69,
					al_map_rgb(255, 255, 255), 18, 34.5, pos.getX(), pos.getY(), 0.5, 0.5, -angulo, 0);
			}
			else{
				al_draw_tinted_scaled_rotated_bitmap_region(spriteLaser, timerAnimacaoAndar.getContador()/3*63, 0, 63, 69,
					al_map_rgb(255, 255, 255), 18, 34.5, pos.getX(), pos.getY(), 0.5, 0.5, -angulo, 0);
				if(!timerAnimacaoAndar.estaAtivo())
			       timerAnimacaoAndar.start();
			}
...
ALLEGRO_BITMAP* Player::spriteSniper;
ALLEGRO_BITMAP* Player::spriteLaser;
ALLEGRO_BITMAP* Player::spriteRir;
ALLEGRO_BITMAP* Player::spriteChorar;
ALLEGRO_BITMAP* Player::spriteNeutro;
...
void Player::initImagens(){
	...
	spriteSniper = al_load_bitmap("imagens/player_sniper.png");
	spriteLaser = al_load_bitmap("imagens/player_laser.png");
	spriteRir = al_load_bitmap("imagens/player_rir.png");
	spriteChorar = al_load_bitmap("imagens/player_chorar.png");
	spriteNeutro = al_load_bitmap("imagens/player_neutro.png");
}
